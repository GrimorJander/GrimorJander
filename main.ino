...
#define MAX_RUNNERS 10     // Define el número máximo de corredores
  ...
  void onWrite(BLECharacteristic *pCharacteristic) {
    ...
    if (rxValue.length() > 0) {
      ...
      if (rxValue == "bat") {
        ...
        snprintf(msgBuffer, sizeof(msgBuffer), "Crono V=%d \nLaser V=%d\n", localVoltage, remoteBatteryMv);
        ...
        if (num >= 2 && num <= MAX_RUNNERS) {
          ...
          if (deviceConnected) {
            String message = "Numero de corredores: " + String(runners) + ". Listo para empezar.\n";
          ...
          if (deviceConnected) {
            pTxCharacteristic->setValue("Numero invalido. Por favor, 2-10.\n");
            pTxCharacteristic->notify();
          }
        }
        drawTime(0);
  ...
  if (currentMode == SOLO || currentMode == SPLIT) {
    ...
    if (currentMode == SPLIT) {
      ...
      if (hasBestTime) {
        ...
        display.drawString(start_x + run_icon_width + 4, 0, "SOLO");
        display.setTextAlignment(TEXT_ALIGN_CENTER);
      }
    }
    ...
    if (multiState == WAITING_FOR_RUNNERS) {
      ...
      display.drawString(start_x + 3 * (run_icon_width + icon_spacing), text_y, "MULTI");

      // Dibujar la segunda línea 6 píxeles más abajo
      display.setTextAlignment(TEXT_ALIGN_CENTER);
      display.drawString(display.width() / 2, 30, "Runners Number?");
      ...
      display.setTextAlignment(TEXT_ALIGN_CENTER);
      display.setFont(ArialMT_Plain_16);
      display.drawString(display.width() / 2, 20, "Runners:  " + String(runners));
      display.drawString(display.width() / 2, 40, "Ready to start");
    } else if (multiState == RACE_IN_PROGRESS) {
      display.drawString(display.width() / 2, 0, "RUNNING: " + String(runnersFinished + 1) + "/" + String(runners));
      display.setFont(ArialMT_Plain_24);
      unsigned long totalSeconds = time / 1000;
      unsigned long centiseconds = (time % 1000) / 10;
  ...
  if (currentMode == SPLIT) {
    ...
    if (!isRunning) {
      ...
      if (deviceConnected) {
        ...
        char msgBuffer[80];
        snprintf(msgBuffer, sizeof(msgBuffer), "SPLIT: %s    PB: %s %s\n",
  ...
  if (currentMode == SOLO) {
    ...
    if (multiState == RACE_IN_PROGRESS) {
      ...
      if (runnersFinished >= runners) {
        ...
        if (deviceConnected) {
          pTxCharacteristic->setValue("Race Complete.\n");
          pTxCharacteristic->notify();
        }
      } else {
        drawTime(lapTime);
...
void processBLECommand(String cmd) {
  if (cmd == "start") {
    if (!isRunning) {
      if (currentMode == SOLO || (currentMode == MULTI && (multiState == READY_TO_START || multiState == RACE_FINISHED))) {
        // Inicia la secuencia de pitido simple
        triggerBeep(SINGLE_BEEP);

        startTime = millis();
        isRunning = true;
        digitalWrite(LED_PIN, HIGH); // --> [NUEVO] Encender el LED
